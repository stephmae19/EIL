...
class Camera:
    ...
    def apply_rect(self, rect):
        """Helper to apply camera offset to a generic rect (for UI/Text)"""
...
# --- BOOKSHELF INTERACTION ZONE ---
class BookshelfZone:
    def __init__(self, x, bottom_y, flavor_text):
        # The interaction zone is 150px wide and 250px tall
...
class Player(pygame.sprite.Sprite):
    ...
    def load_frames(self, filename, rows, cols):
        ...
        # Safely handling PixelArray with comma indexing
        ...
        for x in range(sheet.get_width()):
            for y in range(sheet.get_height()):
                ...
                if color.r < JPG_BLACK_TOLERANCE and color.g < JPG_BLACK_TOLERANCE and color.b < JPG_BLACK_TOLERANCE:
                    pixel_array[x, y] = 0x00000000 # Hex for transparent black
        del pixel_array 
...
bookshelves = [
    ...
    BookshelfZone(x=1800, bottom_y=floor_y, flavor_text="Nothing but crumbled parchment here."),
    BookshelfZone(x=2300, bottom_y=floor_y, flavor_text="You found a book titled 'A Guide to Pygame'.")
]
