...
class Player(pygame.sprite.Sprite):
    ...
    def update(self, map_width):
        ...
        self.animate()
        # Keep player solidly on the floor
...
class Enemy(pygame.sprite.Sprite):
    def __init__(self, start_x, floor_y, target_width, target_height):
        ...
        # Apply the unique offset just to the enemy
    ...
    def load_frames(self, filename, rows, cols):
        ...
        try:
            ...
            # --- THE "LUMINANCE BANDING" FIX ---
            for x in range(sheet.get_width()):
                for y in range(sheet.get_height()):
                    color = sheet.unmap_rgb(pixel_array[x, y])
                    r, g, b = color.r, color.g, color.b
                    
                    # 1. Glowing Details (Keep Bright)
                    if r > 120 and g > 120 and b > 120:
                        pass 
                        
                    # 2. Pitch Black Body (Force Solid Black)
                    elif r < 18 and g < 18 and b < 18:
                        pixel_array[x, y] = (0, 0, 0, 255) 
                        
                    # 3. Mid-Tones (Background vs. Outline)
                    else:
                        # If it's somewhat bright, it's the monster's glitchy outline.
                        if r > 45 or g > 45 or b > 45:
                            pixel_array[x, y] = (0, 0, 0, 255) # Turn outline to solid black
                        # If it is between 18 and 45, it is the muddy JPEG background noise.
                        else:
                            pixel_array[x, y] = (0, 0, 0, 0) # Make completely invisible
    ...
    def update(self, player, projectile_group):
        ...
        # Lock enemy strictly to its unique offset floor line
        self.rect.midbottom = (self.rect.centerx, self.floor_y)
