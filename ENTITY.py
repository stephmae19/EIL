...
class Enemy(pygame.sprite.Sprite):
    ...
    def load_frames(self, filename, rows, cols):
        ...
        try:
            ...
            # --- THE "TOTAL BRIGHTNESS" FIX ---
            for x in range(sheet.get_width()):
                for y in range(sheet.get_height()):
                    color = sheet.unmap_rgb(pixel_array[x, y])
                    r, g, b = color.r, color.g, color.b
                    
                    # Calculate total brightness of the pixel
                    total_brightness = r + g + b
                    
                    # 1. Glowing Details (Keep Bright) -> High brightness
                    if total_brightness > 200:
                        pass # Leave it alone
                        
                    # 2. Pitch Black Body (Force Solid Black) -> Extreme low brightness
                    elif total_brightness < 60:
                        pixel_array[x, y] = (0, 0, 0, 255) # Solid, opaque black
                        
                    # 3. The Muddy Background Noise -> Mid brightness
                    else:
                        pixel_array[x, y] = (0, 0, 0, 0) # 100% Invisible
