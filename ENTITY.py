        ...
        try:
            ...
            for x in range(sheet.get_width()):
                for y in range(sheet.get_height()):
                    ...
                    # Remove player background (dark pixels)
...
class Enemy(pygame.sprite.Sprite):
    ...
    def load_frames(self, filename, rows, cols):
        ...
        try:
            ...
            # --- ADVANCED FLOOD FILL ALGORITHM ---
            # 1. Grab the exact color of the top-left background pixel
            bg_color = sheet.unmap_rgb(pixel_array[0, 0])
            tolerance = 30
            
            # We use a stack to track which pixels to delete (acting like water pouring in)
            stack = []
            visited = set()
            
            # Start pouring water from all 4 borders of the image
            for x in range(w):
                stack.append((x, 0))
                stack.append((x, h - 1))
                visited.add((x, 0))
            ...
            while stack:
                cx, cy = stack.pop()
                color = sheet.unmap_rgb(pixel_array[cx, cy])
                
                # If this pixel matches the background color...
                if abs(color.r - bg_color.r) <= tolerance and \
                   abs(color.g - bg_color.g) <= tolerance and \
                   abs(color.b - bg_color.b) <= tolerance:
                    
                    # Delete it (Make it transparent)
                    pixel_array[cx, cy] = (0, 0, 0, 0)
                    
                    # Look at the neighboring pixels to see if they should be deleted too
            ...
            # 2. PAINT THE CREATURE
            # The flood fill stopped at the creature's outline, meaning everything inside was protected!
            for x in range(w):
                for y in range(h):
                    color = sheet.unmap_rgb(pixel_array[x, y])
                    
                    # If the pixel was NOT deleted by the flood fill...
                    if color.a > 0:
                        # Keep it if it's a bright glowing pixel (eyes/blast)
                        if color.r > 130 or color.g > 130 or color.b > 130:
                            pass 
                        # Otherwise, force the outline AND the protected inside to be SOLID PURE BLACK
                        else:
                            pixel_array[x, y] = (0, 0, 0, 255)
                            
            del pixel_array 
            
            # 3. Cut the grid into frames
            w_frame = w // cols
            h_frame = h // rows
            frames = []
            for r in range(rows):
                for c in range(cols):
                    original = sheet.subsurface(pygame.Rect(c * w_frame, r * h_frame, w_frame, h_frame))
