...
# Nudge the enemy up/down to perfectly match the player's tiles
ENEMY_Y_OFFSET = -15 
...
class Enemy(pygame.sprite.Sprite):
    ...
    def load_frames(self, filename, rows, cols):
        ...
        try:
            ...
            # --- THE BLACK BOX KILLER LOGIC ---
            # Grab the exact background color from the top left pixel
            ...
            for x in range(sheet.get_width()):
                for y in range(sheet.get_height()):
                    ...
                    # 1. Is it the background? (High tolerance to kill JPEG noise blocks)
                    if abs(color.r - bg_color.r) < 60 and abs(color.g - bg_color.g) < 60 and abs(color.b - bg_color.b) < 60:
                        pixel_array[x, y] = (0, 0, 0, 0) # 100% Invisible Background
                        
                    # 2. Is it a bright glowing piece (eyes/blast)? Keep it!
                    elif color.r > 130 and color.g > 130 and color.b > 130:
                        pass 
                        
                    # 3. Everything else MUST be the creature's body. Make it SOLID BLACK.
                    else:
                        pixel_array[x, y] = (0, 0, 0, 255) # Pure Opaque Black
